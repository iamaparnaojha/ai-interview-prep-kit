...
export function allocateSchedule(requirements: Requirement[], questions: Question[], daysAvailable: number): ScheduleDay[] {
  if (!Number.isInteger(daysAvailable) || daysAvailable < 1 || daysAvailable > 60) throw new Error('daysAvailable must be an integer from 1 to 60');
  ...
  const buckets = Array.from({ length: daysAvailable }, () => [] as Question[]);
